...
export default function ResourcesPage() {
  ...
  const workloadMap = {};
  users.forEach((u) => {
    workloadMap[u.id] = { user: u, tasks: [], totalHours: 0 };
  });
  tasks
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p>
            {currentProject?.project?.name} · {users.length} team members
          </p>
        </div>
      ...
      {activeTab === "workload" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {users.map((u) => {
            ...
            const blocked =
              load?.tasks?.filter((t) => t.status === "blocked")?.length || 0;
            const overdue =
              load?.tasks?.filter(
                (t) =>
                  t.due_date &&
          ...
          {!optimization ? (
            <div className="card empty-state" style={{ minHeight: 300 }}>
              ...
              >
                {running ? (
                  ...
                  <>
                    <Zap size={16} /> Optimize Resources
                  </>
                )}
              </button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, rgba(79,142,247,0.08), rgba(16,185,129,0.08))",
                }}
              >
                <div
                  style={{
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 800,
                        color:
                          optimization.optimization_score > 70
                            ? "var(--success)"
                            : "var(--warning)",
                      }}
                    >
                  ...
                  {optimization.recommendations.map((r, i) => (
                    <div
                      ...
                      style={{
                        padding: "8px 0",
                        borderBottom:
                          i < optimization.recommendations.length - 1
            ...
            <div className="card empty-state" style={{ minHeight: 300 }}>
              ...
              >
                {running ? (
                  ...
                  <>
                    <TrendingUp size={16} /> Analyze Team
                  </>
                )}
              </button>
            </div>
                  ...
                  >
                    <div className="avatar" style={{ overflow: "hidden" }}>
                      {insight.avatar ? (
                        ...
                        insight.name?.substring(0, 2).toUpperCase()
                      )}
                    </div>
                    <div>
                      <div
                      ...
                      <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
                        ...
                        <span
                          style={{
                            color:
                              insight.on_time_rate > 0.75
                                ? "var(--success)"
                                : "var(--warning)",
                            fontWeight: 700,
                          }}
                        >
                          {Math.round(insight.on_time_rate * 100)}%
                        </span>
                        {insight.avg_delay_days > 0 &&
                          ` · Avg delay: ${insight.avg_delay_days}d`}
                      </div>
                    </div>
                  </div>
              ...
              {insights.predictive_alerts?.length > 0 && (
                ...
                >
                  <div
                    ...
                    style={{ marginBottom: 12, color: "var(--warning)" }}
                  >
                    ⚠ Predictive Alerts
                  </div>
                  {insights.predictive_alerts.map((a, i) => (
                    <div
                      key={i}
                      style={{
                        fontSize: 14,
