...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", (msg) => {
    try {
      ...
      if (data.type === "ping") {
        ...
        broadcast(data);
      } else if (data.type === "screen_share_data") {
        // Optimize: skip complex logging for high-freq data
        broadcast(data);
      } else if (data.type === "chat_message") {
        const { query, queryOne } = require("./database");
        const messageId = uuidv4();
        query(
          "INSERT INTO messages (id, project_id, conversation_id, author_id, content, type, file_url, reply_to) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
          [
            messageId,
            data.projectId || null,
        ...
        query("UPDATE messages SET is_pinned = $1 WHERE id = $2", [
          data.isPinned,
          data.messageId,
        ]).then(() => {
          broadcast({
        ...
        const { targetId } = data;
        if (targetId && app.locals.broadcastToUser) {
          app.locals.broadcastToUser(targetId, {
            ...data,
            senderId: userId,
  ...
  ws.send(
    JSON.stringify({
      type: "connected",
      message: "CWB Azure Intelligence connected",
    }),
  );
});
...
function broadcastToUser(userId, data) {
  ...
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    console.log(`[WS] Targeted message sent to ${userId}: ${data.type}`);
  } else {
    console.log(`[WS] Target user ${userId} not connected or socket closed`);
  }
}
  ...
  try {
    ...
    for (const project of projects) {
      ...
      await query(
        ...
        [analysis.project_risk_score, analysis.health_score, project.id],
      );

      if (analysis.project_risk_score > 70) {
        const pm = await queryOne(
          "SELECT id FROM users WHERE role IN ('project_manager', 'admin') LIMIT 1",
        );
        if (pm) {
          await query(
            "INSERT INTO notifications (id, user_id, type, title, message, priority) VALUES ($1, $2, $3, $4, $5, $6)",
            [
              uuidv4(),
              pm.id,
              "risk_alert",
              `⚠️ High Risk: ${project.name}`,
              `Risk score: ${analysis.project_risk_score}/100`,
              "urgent",
            ],
          );
          broadcastToUser(pm.id, {
            type: "notification",
            data: { title: "High Risk Alert", project: project.name },
          });
        }
      }
      results.push({
        projectId: project.id,
        riskScore: analysis.project_risk_score,
      });
    }
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    services: {
      ai_foundry: !!process.env.AZURE_OPENAI_ENDPOINT
        ? "✅ Azure AI Foundry"
        : "⚡ Rule-based mode",
      ...
      cosmos: !!process.env.COSMOS_ENDPOINT
        ? "✅ Azure Cosmos DB"
        : "⚠️ Disabled",
      blob_storage: !!process.env.AZURE_STORAGE_CONNECTION_STRING
        ? "✅ Azure Blob Storage"
        : "⚠️ Disabled",
...
app.post("/api/seed", async (req, res) => {
  try {
    ...
    for (const [i, t] of tasks.entries()) {
      await query(
        `INSERT INTO tasks (id, project_id, title, status, priority, owner_id, due_date, baseline_due_date, estimated_hours, completion_pct, position, source)
    ...
    res.json({
      success: true,
      message: "Demo data seeded. Login: admin@cwb.com / password123",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
